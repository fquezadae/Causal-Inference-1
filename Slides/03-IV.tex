...
\date{(Slides de Scott Cunningham, Baylor University )}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
\begin{frame}[plain]
...
\section{Grafos Acíclicos Dirigidos}

\begin{frame}{Ajustar por observables}

\begin{itemize}

\item Una de las principales formas en que se centraba la parte de "diseño" de esta rama era "ajustar por observables" que podrían estar violando la independencia.
\item Este trabajo se remonta a mucho tiempo atrás, incluso hasta Yule en el siglo XIX, quien utilizó regresiones con controles.
\item Pero ahora queremos ofrecer una revisión más moderna y completa de este material.
\item Usaremos grafos para ayudarnos a entender mejor cuándo podemos eliminar el sesgo de selección simplemente incluyendo controles observables, o lo que se llama "no confusión", "ignorabilidad", "selección sobre observables" o "exogeneidad".
...
\subsection{Notación de grafos}

\begin{frame}{Judea Pearl y los DAGs}

\begin{itemize}
    \item Judea Pearl y sus colegas en Inteligencia Artificial en la UCLA desarrollaron los modelos de grafos acíclicos dirigidos (DAGs) para crear una metodología formalizada de inferencia causal.
    \item Las letras significan lo siguiente:
    	\begin{itemize}
	\item \textbf{D}: Dirigido. Esto es una referencia a la teoría de grafos. Los bordes directos representan caminos causales de una variable a otra.
	\item \textbf{A}: Acíclico. Esto significa que el camino causal solo se mueve en una dirección. No hay ciclos ni retroalimentación. Limita su capacidad para representar muchas cosas en economía (por ejemplo, causalidad inversa, ecuaciones simultáneas).
	\item \textbf{G}: Grafos. Y esto también es una referencia a la teoría de grafos. Se representan con un sistema de nodos y flechas.
	\end{itemize}
    \item Usaremos estos para entender la selección sobre observables ("no confusión") y variables instrumentales.
\end{itemize}
...
\begin{frame}{Modelo causal}

\begin{itemize}
    \item Los DAGs utilizan la información previa que tienes sobre la asignación del tratamiento y los determinantes del resultado para modelar los diferentes canales (tanto directos como a través de "puertas traseras") por los cuales el tratamiento causa el resultado.
	\item Si tu conocimiento del DAG es correcto, entonces la identificación de los efectos promedio del tratamiento se vuelve sencilla — pero hay pocas formas de "probar" el DAG, por lo que su popularidad entre los economistas ha sido recibida con escepticismo.
	\item Sin embargo, para la selección de covariables que se utilizarán en cualquier método que controle por covariables, son indispensables.
    \item Considera el siguiente diagrama que representa los retornos de la educación con factores de confusión simplificados.
\end{itemize}
...
\begin{frame}[plain]
  ...
  \bigskip
  \begin{itemize}
    \item $B$ es un \textbf{progenitor} de $PE$ y $D$
    \item $PE$ y $D$ son \textbf{descendientes} de $B$
    \item Existe un \textbf{camino directo (causal)} de $D$ a $Y$
    \item Existe un \textbf{camino mediado (causal)} de $B$ a $Y$ a través de $D$
    \item Hay muchos \textbf{caminos} de $PE$ a $Y$, pero ninguno es directo, y algunos van "hacia abajo" mientras que otros van "hacia arriba"
  \end{itemize}
\end{frame}
...
\begin{frame}{Creando tu DAG}

	\begin{itemize}
	\item Crea un DAG que sea una aproximación razonable y honesta de los progenitores (confusores) de $D$ y $Y$, así como de los efectos directos e indirectos de $D$ sobre $Y$
	\item Donde haya incertidumbre, debes reconocerla en el DAG, y donde haya creencias, también debes mencionarlas
	\item Obtenemos ideas para los DAGs a partir de la teoría, modelos, observación, experiencia, estudios previos, intuición, así como conversaciones con expertos en el campo
	\end{itemize}

\end{frame}


\begin{frame}{Etapa Previa de Análisis de los DAGs}

	\begin{itemize}
	\item Los DAGs están destinados a representar la verdad sobre los mecanismos de asignación de tratamiento y la selección de las unidades en \emph{tus datos}
	\item Pero crear los DAGs después de recopilar los datos puede ser hacer trampa, ya que puedes dibujar el DAG simplemente basado en lo que tienes en lugar de lo que es cierto
	\item Lo que determina el tratamiento en \emph{tu conjunto de datos} puede ser diferente de lo que lo determina en el de otra persona, incluso si el tema es el mismo
	\item Deberás dedicar tiempo antes de tus proyectos a entender por qué las unidades en tus datos participaron en el tratamiento; esto no es algo que se hace después de obtener los datos
	\end{itemize}
\end{frame}
...
\subsection{Criterio de puerta trasera}


\begin{frame}{Confusión}

  \begin{itemize}
  	\item Las covariables que necesitas controlar bajo no confusión se llaman \textbf{confusores}, ya que los confusores crean caminos de puerta trasera desde el tratamiento hasta el resultado
    \item Los confusores crean sesgo de selección \\
          ...
          \item Nuestro conocimiento sobre esto nos permitirá seleccionar las covariables correctas (pero no necesariamente la especificación correcta)
          % \item Vamos a ver \texttt{confounders.do} en nuestros laboratorios compartidos (/Labs/DAGs)
  \end{itemize}
\end{frame}

\begin{frame}{Caminos de puerta trasera}

  \begin{itemize}
    \item La confusión crea \textbf{caminos de puerta trasera} entre el tratamiento y el resultado ($D\leftarrow X\rightarrow Y$), es decir, correlaciones espurias
		\begin{itemize}
		\item Distinto de lo que se llama un camino de colisionador ($D \rightarrow X \leftarrow Y$)
		\item Distinto de lo que se llama un camino mediador ($D \rightarrow X \rightarrow Y$)
		\end{itemize}
    \item Podemos "bloquear" cualquier camino de puerta trasera particular condicionando en la variable $X$ siempre que no sea un colisionador (visualizado aquí con un cuadrado sobre $X$)
...
\begin{frame}{Caminos de puerta trasera}
  ...
  \begin{itemize}
    \item Una vez que condicionamos en $X$, podemos calcular las diferencias promedio en $Y$ según el estado del tratamiento para obtener una estimación de un parámetro causal agregado
    \item Hay muchos métodos para hacer esto: regresión, emparejamiento, estratificación, ponderación
    \item Pero todos, en su nivel fundamental, calculan diferencias simples en los resultados promedio para los valores dados de $X$ y luego toman promedios ponderados
...
\begin{frame}{Caminos de puerta trasera}
  ...
  \begin{itemize}
	...
	\item Cuando todos los caminos de puerta trasera de $D$ a $Y$ están bloqueados, entonces el único camino restante entre $D$ y $Y$ es el camino causal
	\item A esto lo llamamos cumplir con el criterio de puerta trasera usando la terminología de los DAG de Pearl, y lo llamamos no confusión usando la terminología de resultados potenciales de Rubin (que discutiré más adelante)
...
\begin{frame}{Criterio de puerta trasera}

  \begin{block}{Criterio de puerta trasera}
    Condicionar en $X$ satisface el criterio de puerta trasera con respecto a la trayectoria dirigida $(D,Y)$ si:
    \begin{enumerate}
      \item Todos los caminos de puerta trasera están bloqueados por $X$
      \item Ningún elemento de $X$ es un colisionador
    \end{enumerate}

  \end{block}
  
       Condicionar en un no-colisionador es suficiente para cerrar un camino de puerta trasera, incluso si se ha abierto condicionando en un colisionador. Por lo tanto, asegúrate de cerrar un camino de puerta trasera si lo abriste con un colisionador.

\end{frame}


\begin{frame}{¿Qué estrategia de control satisface el criterio de puerta trasera?}
  \begin{itemize}
    \item Enumera todos los caminos de puerta trasera desde $D$ a $Y$. Espero.
    ...
    \item ¿Cuáles son los controles necesarios y suficientes que satisfacen el criterio de puerta trasera?
  \end{itemize}
...
\begin{frame}{¿Qué sucede si tienes una variable no observable?}

  \begin{itemize}
    \item Enumera todos los caminos de puerta trasera desde $D$ a $Y$.
    ...
    \item ¿Cuáles son los controles necesarios y suficientes que satisfacen el criterio de puerta trasera?
    \item ¿Qué sucede con la variable no observable, $U$?
  \end{itemize}
...
\begin{frame}{Múltiples estrategias}
  ...
  \begin{itemize}
  \tiny
    \item Condicionar en las causas comunes, $X_1$ y $X_2$, es suficiente, pero también lo es condicionar en $X_3$
    % \item Consulta \texttt{confounder_multiple.do} para ver las diferencias en la varianza; el sobrecontrol es más preciso
  \end{itemize}
\end{frame}


\subsection{Sesgo de colisionador}

\begin{frame}{Sesgo de colisionador}

\begin{itemize}
\item Los caminos de puerta trasera pueden permanecer abiertos en las estrategias de ajuste de covariables de dos formas:
	\begin{enumerate}
	\item No cerraste el camino porque no condicionaste en el confundidor
	\item Tu variable de control abrió un camino de puerta trasera previamente cerrado porque en ese camino la variable era un \textbf{colisionador}
	\end{enumerate}
\item Los colisionadores son "malos controles" que, cuando los controlas, \emph{crean} nuevas correlaciones espurias previamente inexistentes (no es un tema comúnmente discutido, incluso en economía)
\item Este es el riesgo de controlar variables a ciegas: podrías incluir inadvertidamente controles malos o irrelevantes
\end{itemize}

\end{frame}


\begin{frame}{Ejemplo 1: Estrellas de cine}

  \alert{Importante}: Dado que los colisionadores sin condicionar bloquean los caminos de puerta trasera, ¿qué hace exactamente el condicionar sobre un colisionador? Vamos a ilustrarlo con un ejemplo y algunos datos inventados\\
  \begin{itemize}
    \item \underline{Titular de CNN.com}: Megan Fox elegida la peor -- pero la más sexy -- actriz de 2009 \myurlshort{http://marquee.blogs.cnn.com/2009/12/30/megan-fox-voted-worst-but-sexiest-actress-of-2009/}{(enlace)}
    \item ¿Estas dos cosas están realmente correlacionadas de forma negativa en el mundo?
    \item Supongamos que el talento y la belleza son independientes, pero cada uno causa que alguien se convierta en una estrella de cine. ¿Cuál es la correlación entre talento y belleza en una muestra de estrellas de cine en comparación con la población en general (estrellas y no estrellas)?
  \end{itemize}

\end{frame}

\begin{frame}{DAG de estrella de cine}

Imagina que los directores de casting eligen a las estrellas de cine basándose en el talento y la belleza.
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      %nodos
      \node[text centered,draw,rectangle,thin] (x) {Estrella de cine};
      \node[below left of = x] (u1) {$Talento$};
      \node[below right of = x] (u2) {$Belleza$};
...
El talento y la belleza pueden volverse correlacionados aunque sean independientes; véase \texttt{moviestar.R}
...
\begin{frame}[shrink=20,plain]
  ...
  \begin{figure}
    ...
    \caption{Figura superior izquierda: Diagrama de dispersión de una muestra de no estrellas de belleza (eje vertical) y talento (eje horizontal). Figura superior derecha: Diagrama de dispersión de una muestra de estrellas de belleza y talento. Figura inferior izquierda: Diagrama de dispersión de una muestra total (estrellas y no estrellas) de belleza y talento.}
  \end{figure}
\end{frame}

\begin{frame}{¿Selección de muestra?}

\begin{itemize}
\item Noten que esto es claro cuando nos enfocamos en la selección de muestra.
\item Pero incluso una regresión que incluya ``estrella'' crearía el problema: $$belleza_i = \alpha + \delta talento_i + \beta estrella_i + \varepsilon_i$$
\item No es solo selección de muestra.
\end{itemize}

\end{frame}

\begin{frame}{Ejemplo 2: Discriminación}

  \begin{itemize}
    \item Veamos otro ejemplo: es muy común que los think tanks y los periodistas digan que la brecha de género en los salarios desaparece una vez que se controla por ocupación.
    \item Pero, ¿y si la ocupación es un colisionador, lo cual podría ser en un modelo con clasificación ocupacional?
    \item Entonces, controlar por ocupación en una regresión salarial que busca discriminación puede llevar a todo tipo de resultados extraños, \emph{incluso en una simulación en la que diseñamos explícitamente que haya discriminación}.
  \end{itemize}
...
\begin{frame}{DAG}
  ...
  $F$ es femenino, $d$ es discriminación, $o$ es ocupación, $y$ son ingresos y $A$ es habilidad. Las líneas discontinuas indican que la variable no puede observarse. Noten que, por diseño, ser mujer no tiene efecto sobre los ingresos o la ocupación, y no tiene relación con la habilidad. Así que los ingresos se explican a través de discriminación, ocupación y habilidad.
...
\begin{frame}[plain]
  ...
  Mediación y Caminos de puerta trasera
...
\begin{frame}[plain, shrink=20]
  ...
  \begin{table}[htbp]\centering
    \scriptsize
    \caption{Regresiones que ilustran sesgo de colisionador con disparidad de género simulada}
    \begin{center}
      \begin{tabular}{l*{3}{c}}
        \toprule
        \multicolumn{1}{l}{Covariables: }&
        \multicolumn{1}{c}{\textbf{Efecto combinado no sesgado}}&
        \multicolumn{1}{c}{\textbf{Sesgado}}&
        \multicolumn{1}{c}{\textbf{Solo efecto no sesgado del salario}}\\
        \midrule
        Mujer                      & -3.074*** & 0.601*** & -0.994*** \\
                                   & (0.000)   & (0.000)  & (0.000)   \\
        Ocupación                  &           & 1.793*** & 0.991***  \\
                                   &           & (0.000)  & (0.000)   \\
        Habilidad                  &           &          & 2.017***  \\
                                   &           &          & (0.000)   \\
        \\
        ...
        Media de la variable dependiente & 0.45      & 0.45     & 0.45      \\
        \bottomrule
      \end{tabular}
    \end{center}
  \end{table}
  ...
  \begin{itemize}
    \item Recordemos que diseñamos el coeficiente de discriminación para ser -1
    \item Si no controlamos por ocupación, obtenemos el efecto combinado de $d \rightarrow o \rightarrow y$ y $d  \rightarrow y$
    \item Aunque parece intuitivo controlar por ocupación, observa la columna 2: ¡el signo cambia!
    \item Solo podemos aislar el efecto causal directo condicionando sobre habilidad y ocupación, pero la habilidad no se observa
  \end{itemize}
...
\begin{frame}[allowframebreaks,plain]
  ...
  \begin{itemize}
    \item \textbf{Los colisionadores pueden ser resultados (y a menudo lo son)}
          \begin{itemize}
            \item Solo hay un camino de puerta trasera de $D$ a $Y$
            ...
            \item Condicionar en $X_1$ bloquea el camino de puerta trasera


            \item Pero, ¿qué pasa si también condicionamos en $X_2$?
            ...
            \item Condicionar en $X_2$ abre un nuevo camino, creando correlaciones espurias entre $D$ y $Y$ 
          \end{itemize}

          \framebreak


    \item \textbf{Los colisionadores pueden ser covariables pre-tratamiento (llamado sesgo en M porque parece una M)}
          \begin{itemize}
            \item Nombra los caminos de puerta trasera. ¿Están abiertos o cerrados?
            ...
            \item Pero, ¿qué pasa si condicionamos en $X$?
...
\begin{frame}{Probando la Validez del DAG}

  \begin{itemize}
    \item El DAG hace predicciones comprobables
    \item Condicionado a $D$ e $I$, la educación parental ($PE$) ya no debería estar correlacionada con $Y$
    \item Puede ser difícil descifrar esto a mano, pero el software puede ayudar (por ejemplo, Daggity.net es basado en navegador, Causal Fusion es más avanzado)
    \item Los algoritmos causales tienden a estar basados en DAG y se están volviendo populares en la industria
  \end{itemize}
...
\begin{frame}{Contraste con las prácticas ordinarias}

  \begin{itemize}
    \item Las personas intentan ``controlar el sesgo por variables omitidas'' al incluir la mayor cantidad posible de ``controles''
    \item No intentan pensar en el mecanismo de asignación del tratamiento, por lo que no tienen idea de qué variables son colisionadores, covariables o factores de confusión
    \item El aprendizaje automático también puede ser ``ingenuo'' si la gran dimensión de características incluye colisionadores de manera inadvertida
  \end{itemize}
...
% \begin{frame}{Selección de covariables sin DAGs}

%   \begin{itemize}
%     \item ¿Qué sucede si no tienes un DAG en el que te sientas confiado?
%       \begin{enumerate}
%         \item Incluye factores de confusión que creas que con bastante seguridad están presentes
%         \item Incluye covariables que sean \emph{altamente predictivas} del contrafactual faltante (por ejemplo, $Y^0$ para el ATT)
%         \item Evita los resultados (aunque esto no resolverá los colisionadores de sesgo M)
%       \end{enumerate}
%     \item Aunque este enfoque puede estar menos formalizado, al menos estás razonando sobre el mecanismo de asignación del tratamiento en lugar de incluir cualquier variable que tengas a mano (evita las ``regresiones de cocina de todo'')
%   \end{itemize}
...
\begin{itemize} \item A menudo se asocian con ``experimentos naturales'' porque un instrumento es cuando descubrimos que nuestra variable de tratamiento cambió aleatoriamente cuando ocurrió un tercer evento aparentemente inconsecuente. \item Los experimentos naturales rara vez son ``buenos instrumentos'' porque suelen estar asociados con demasiada destrucción para ser útiles, pero la metáfora puede ser útil, ya que a menudo usamos instrumentos para recrear las condiciones de un experimento aleatorio fuera del laboratorio. \item ¿Encontramos los instrumentos o ellos nos encuentran a nosotros? Tienden a volverse evidentes cuando entendemos lo que son, ya que de otro modo son como camaleones que se mezclan con el fondo.
...
\begin{frame}{¿Como Scott Cunningham ve la enseñanza de IV?}

\begin{enumerate}
    \item Intuición primero -- ¿qué es un instrumento y qué se hace con él?
    \item Estimación después -- ¿cuáles son los métodos más comunes empleados?
    \item Interpretación -- efectos de tratamiento constantes versus efectos de tratamiento heterogéneos y el parámetro LATE
    \item Aplicaciones -- elasticidad precio de la demanda, indulgencia
\end{enumerate}
...
\begin{frame}{Intuición de 2SLS}
    \begin{itemize}
        \item La intuición es que 2SLS reemplaza $S$ con los valores ajustados $\widehat{S}$ de la regresión de primera etapa de $S$ sobre $Z$ y todas las demás covariables.
        \item Nuestras diapositivas anteriores mostraron que 2SLS y la razón de forma reducida y primera etapa eran equivalentes.
        \item Al usar los valores ajustados del regresor endógeno de la regresión de primera etapa, nuestra regresión ahora utiliza \emph{solo} la variación cuasi-aleatoria en el tratamiento debido a la variable instrumental misma (solo quedan las partes aleatorias de la educación).
    \end{itemize}
\end{frame}
...
\item Esto se evidenció con el estudio de trimestre de nacimiento de Angrist y Krueger (1991) y algunos trabajos que siguieron.
\end{itemize}
...
\begin{frame}{Angrist y Krueger (1991)}
	...
	\item Recuerda el ``principio de extrañeza'' - ¿por qué el trimestre de nacimiento causaría ingresos en la forma reducida?
	\end{itemize}
